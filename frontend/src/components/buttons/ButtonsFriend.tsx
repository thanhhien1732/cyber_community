...
function getFriendAction(info: { id?: string }, detailUser: { id?: string }, findOneFriend: any): TStatusResult {
    ...
    let targetStatus: TStatusFriend = "pending";
    let text = "Kết bạn";
    let disabled = false;
    ...
    if (!info?.id || !detailUser?.id) {
        return { targetStatus, text: "Đăng nhập để kết bạn", disabled: true };
    }
    ...
    return { targetStatus, text, disabled };
}
...
export default function ButtonsFriend({ detailUser }: Props) {
    ...
    const handleMakeFriend = () => {
        ...
        const payload: TfriendshipAction = {
            ...
            status: actionInfo.targetStatus,
        };
        friendStatus.mutate(payload, {
            onSuccess: () => {
                let mes = `Đã gửi lời mời kết bạn`;
                if (actionInfo.targetStatus === "accepted") mes = `Đã chấp nhận kết bạn`;
                if (actionInfo.targetStatus === "removed") mes = `Đã xoá bạn`;
                toast.success(mes);
            },
            onSettled: () => {
                queryClient.invalidateQueries({ queryKey: [`find-one-friend`] });
