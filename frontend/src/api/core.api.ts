...
import { VITE_BASE_DOMAIN_API } from "@/constant/app.constant";
...
export const refreshToken = async () => {
    ...
    try {
        ...
        const res = await fetch(`${VITE_BASE_DOMAIN_API}auth/refresh-token`, {
            method: "POST",
            headers: {
    ...
    private async request<T>(url: string, options: FetchOptions = {}): Promise<T> {
        ...
        if (response.status === 403) {
            ...
            try {
                ...
                response = await fetch(`${VITE_BASE_DOMAIN_API}${url}`, {
                    ...options,
                    headers: {
                        ...(options.headers || {}),
                        Authorization: `Bearer ${accessToken}`,
...
const api = new APIClient(VITE_BASE_DOMAIN_API);
