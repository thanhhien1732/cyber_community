...
import { VITE_BASE_DOMAIN, VITE_BASE_DOMAIN_CLOUDINARY, FOLDER_IMAGE_BE } from "@/constant/app.constant";
...
export const checkPathImage = (path: string | null | undefined) => {
    ...
    if (path.includes(`local`)) {
        return `${VITE_BASE_DOMAIN}${FOLDER_IMAGE_BE}${path}`;
    } else {
        return `${VITE_BASE_DOMAIN_CLOUDINARY}${path}`;
    }
};
